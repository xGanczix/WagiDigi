...
let tydzienwczesniej = new Date();
tydzienwczesniej.setDate(dzisiaj.getDate() - 7);

dzisiaj = dzisiaj.toLocaleDateString("pl-PL");
tydzienwczesniej = tydzienwczesniej.toLocaleDateString("pl-PL");
...
async function fetchTopProdukty(pobierzCSV = false, pobierzPDF = false) {
  try {
    const response = await fetch(`${CONFIG.URL}/api/raporty/top-produkty`);
    const data = await response.json();

    if (pobierzCSV) {
      const rows = [["NazwaTowaru", "SumaWaga"]];
      data.forEach((item) => rows.push([item.NazwaTowaru, item.SumaWaga]));
      downloadCSV(`top_produkty_${tydzienwczesniej}-${dzisiaj}.csv`, rows);
      return;
    }
    ...
    if (pobierzPDF) {
      ...
      downloadPDF(
        `top_produkty_${tydzienwczesniej}-${dzisiaj}.pdf`,
        headers,
        rows
      );
      return;
...
async function fetchTopKasjerzy(pobierzCSV = false, pobierzPDF = false) {
  try {
    const response = await fetch(`${CONFIG.URL}/api/raporty/top-kasjerzy`);
    const data = await response.json();

    if (pobierzCSV) {
      const rows = [["NumerKasjera", "IloscEtykiet"]];
      data.forEach((item) => rows.push([item.NumerKasjera, item.IloscEtykiet]));
      downloadCSV(`top_kasjerzy_${tydzienwczesniej}-${dzisiaj}.csv`, rows);
      return;
    }
    ...
    if (pobierzPDF) {
      ...
      downloadPDF(
        `top_kasjerzy_${tydzienwczesniej}-${dzisiaj}.pdf`,
        headers,
        rows
      );
      return;
...
async function fetchWartoscWagi(pobierzCSV = false, pobierzPDF = false) {
  try {
    const response = await fetch(
      `${CONFIG.URL}/api/raporty/wartosc-sprzedazy-wagi`
    );
    const data = await response.json();
    ...
    if (pobierzCSV) {
      ...
      downloadCSV(`wartosc_wagi_${tydzienwczesniej}-${dzisiaj}.csv`, rows);
      return;
    }
    ...
    if (pobierzPDF) {
      ...
      downloadPDF(
        `wartosc_wagi_${tydzienwczesniej}-${dzisiaj}.pdf`,
        headers,
        rows
      );
      return;
...
async function fetchTopGodziny(pobierzCSV = false, pobierzPDF = false) {
  try {
    const response = await fetch(`${CONFIG.URL}/api/raporty/top-godziny`);
    const data = await response.json();
    ...
    if (pobierzCSV) {
      ...
      downloadCSV(`sprzedaz_godziny_${tydzienwczesniej}-${dzisiaj}.csv`, rows);
      return;
    }
    ...
    if (pobierzPDF) {
      ...
      downloadPDF(
        `sprzedaz_godziny_${tydzienwczesniej}-${dzisiaj}.pdf`,
        headers,
        rows
      );
      return;
...
async function fetchWagaDzienna(pobierzCSV = false, pobierzPDF = false) {
  try {
    const response = await fetch(`${CONFIG.URL}/api/raporty/waga-dzienna`);
    ...
    if (pobierzCSV) {
      ...
      downloadCSV(`waga_dzienna_${tydzienwczesniej}-${dzisiaj}.csv`, rows);
      return;
    }
    ...
    if (pobierzPDF) {
      ...
      downloadPDF(
        `waga_dzienna_${tydzienwczesniej}-${dzisiaj}.pdf`,
        headers,
        rows
      );
      return;
...
async function fetchIloscTransakcji() {
  try {
    const response = await fetch(
      `${CONFIG.URL}/api/raporty/ilosc-transakcji-tydzien`
    );
    const data = await response.json();
...
async function fetchWartoscTransakcji() {
  try {
    const response = await fetch(
      `${CONFIG.URL}/api/raporty/wartosc-transakcji-tydzien`
    );
    const data = await response.json();
...
async function fetchWagaTransakcji() {
  try {
    const response = await fetch(
      `${CONFIG.URL}/api/raporty/waga-transakcji-tydzien`
    );
    const data = await response.json();
...
async function fetchTopDzien() {
  try {
    const response = await fetch(`${CONFIG.URL}/api/raporty/top-dzien`);
    const data = await response.json();
...
async function pngTopProdukty() {
  ...
  try {
    ...
    link.download = `top_produkty_${tydzienwczesniej}-${dzisiaj}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
...
async function pngTopKasjerzy() {
  ...
  try {
    ...
    const link = document.createElement("a");
    link.href = uri.imgURI;
    link.download = `top_kasjerzy_${tydzienwczesniej}-${dzisiaj}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
...
async function pngWagi() {
  ...
  try {
    ...
    const link = document.createElement("a");
    link.href = uri.imgURI;
    link.download = `wartosc_wagi_${tydzienwczesniej}-${dzisiaj}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
...
async function pngTopGodziny() {
  ...
  try {
    ...
    const link = document.createElement("a");
    link.href = uri.imgURI;
    link.download = `sprzedaz_godziny_${tydzienwczesniej}-${dzisiaj}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
...
async function pngWagaDzienna() {
  ...
  try {
    ...
    const link = document.createElement("a");
    link.href = uri.imgURI;
    link.download = `waga_dzienna_${tydzienwczesniej}-${dzisiaj}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
